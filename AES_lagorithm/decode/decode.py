...
def inv_mix_columns(state):
    for i in range(4):
        ...
        state.state[0][i] = multiply(a, 0x0e) ^ multiply(
            b, 0x0b) ^ multiply(c, 0x0d) ^ multiply(d, 0x09)
        state.state[1][i] = multiply(a, 0x09) ^ multiply(
            b, 0x0e) ^ multiply(c, 0x0b) ^ multiply(d, 0x0d)
        state.state[2][i] = multiply(a, 0x0d) ^ multiply(
            b, 0x09) ^ multiply(c, 0x0e) ^ multiply(d, 0x0b)
        state.state[3][i] = multiply(a, 0x0b) ^ multiply(
...
# Copy input to state
    for i in range(4):
        for j in range(4):
            state.state[j][i] = (input_data[i * 4 + j])  # change
...
def hex_to_string(s):
    return [chr(x) for x in s]
# Example usage
# if __name__ == "__main__":
#     # Example input(16 bytes)
#   #  input_data = "ifleisoptdknaoes"
#     input_data = [0x23, 0x3A, 0x21, 0x8F, 0xCF, 0xA, 0x58,
#                   0x25, 0x4D, 0x2E, 0x39, 0x2A, 0x87, 0xB9, 0x19, 0x23]


#     decrypted = aes_decrypt(input_data, key)
#     print("Decrypted:", [chr(x) for x in decrypted])
