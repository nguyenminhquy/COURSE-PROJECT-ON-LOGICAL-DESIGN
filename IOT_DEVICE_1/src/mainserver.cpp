    ...
    <div class="container">
      ...
      <div class="sensor">
        🌡️ Nhiệt độ: <span id="temp">)rawliteral" +
         String(th.temperature) + R"rawliteral(</span> &deg;C
      </div>
      <div class="sensor">
        💧 Độ ẩm: <span id="hum">)rawliteral" +
         String(th.humidity) + R"rawliteral(</span> %
      </div>

      <div>
        <button onclick='toggleLED(1)'>💡 BLUE LED: <span id="l1">)rawliteral" +
         led1 + R"rawliteral(</span></button>
        <button onclick='toggleLED(2)'>💡 GREEN LED: <span id="l2">)rawliteral" +
...
void handleToggle()
{
  ...
  else if (led == 2)
  {
    ...
    digitalWrite(GREEN_LED, (led2_state? HIGH : LOW));
  }
...
void handleSensors()
{
  TempHumid th;
  xQueuePeek(TempHumidQueue, &th, 100);
  String json = "{\"temp\":" + String(th.temperature) + ",\"hum\":" + String(th.humidity) + "}";
